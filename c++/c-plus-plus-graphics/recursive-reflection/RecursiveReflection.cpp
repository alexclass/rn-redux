/*	RecursiveReflection.cpp
	
	This programs implements a recursive routine to produce a reflective effect.  This is accomplished
	through a rewrite of the method render(), to instead implement a recursive ray tracing 
	routine called rayTracer. The implemented recursive method casts a ray from the ray source 
	...
	normal vector of the wall and the floor.
...
// hasIntersection - determines whether a given ray intersects any objects in the scene
bool hasIntersection (vector3 origin, vector3 dir) {
	...
	// checks whether given ray has an intersection in the scene
...
vector3 rayTracer(vector3 origin, vector3 dir, int cumulativeCalls) {
	...
	if ( (!hasIntersection(origin, dir)) || (cumulativeCalls > 5)  ) {
		...
		if( withinObject(wInt, wall) ) { // checks wall
			...
			vector3 rayNorm = reflectiveRay.normalize(); // normalize reflective ray
...
void main(int argc, char **argv) {
	...
	int windowHandle = glutCreateWindow("Recursive Reflection"); // creates window with specified title
